<x-dashboard-layout>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div class="flex flex-col gap-2 rounded-xl p-6 bg-white border border-gray-200 shadow-sm">
            ...
            <p class="text-gray-900 tracking-tight text-3xl font-bold leading-tight">{{ $stats['occupied_classrooms'] }}</p>
        ...
        <div class="flex flex-col gap-2 rounded-xl p-6 bg-white border border-gray-200 shadow-sm">
            ...
            <p class="text-gray-900 tracking-tight text-3xl font-bold leading-tight">{{ $stats['schedule_conflicts'] }}</p>
            <p class="text-yellow-600 text-sm font-medium leading-normal">Por resolver</p>
        </div>
        <div class="flex flex-col gap-2 rounded-xl p-6 bg-white border border-gray-200 shadow-sm">
            <p class="text-gray-700 text-base font-medium leading-normal">Servicio Social Activo</p>
            <p class="text-gray-900 tracking-tight text-3xl font-bold leading-tight">{{ $stats['active_social_service'] }}</p>
            <p class="text-blue-600 text-sm font-medium leading-normal">Estudiantes</p>
        </div>
    </div>
    ...
    <div class="mt-6 bg-white rounded-xl border border-gray-200 shadow-sm p-6 flex flex-col items-start gap-4">
        <div class="flex flex-col">
            ...
            <p class="text-gray-500 text-sm">Ejecuta la asignación automática basada en prioridades o requerimientos específicos.</p>
        </div>
        <div class="flex flex-wrap gap-4">
            <form action="{{ route('dashboard.generate') }}" method="POST">
                @csrf
                <button type="submit" class="flex items-center gap-2 rounded-lg h-10 px-4 bg-primary text-white text-sm font-medium hover:bg-primary/90 transition-all shadow-sm">
            ...
            <form action="{{ route('dashboard.generate-requirements') }}" method="POST">
                @csrf
                <button type="submit" class="flex items-center gap-2 rounded-lg h-10 px-4 bg-green-600 text-white text-sm font-medium hover:bg-green-700 transition-all shadow-sm">
    ...
    <div class="grid grid-cols-1 xl:grid-cols-3 gap-6 mt-6">
        <div class="xl:col-span-2 flex flex-col gap-6">
            ...
            <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                <h2 class="text-gray-900 text-lg font-bold leading-tight tracking-[-0.015em] px-6 pt-5 pb-3">Conflictos de Horario por Resolver</h2>
                <div class="flex flex-col divide-y divide-gray-200">
                    <div class="flex items-center gap-4 px-6 min-h-[72px] py-3 justify-between">
                        <div class="flex items-center gap-4">
                            <div class="text-red-500 flex items-center justify-center rounded-lg bg-red-100 shrink-0 size-12">
                                <span class="material-symbols-outlined text-2xl">error</span>
                            </div>
                            <div class="flex flex-col justify-center">
                                <p class="text-gray-900 text-base font-medium leading-normal line-clamp-1">Doble asignación: Dr. Pérez</p>
                                <p class="text-gray-500 text-sm font-normal leading-normal line-clamp-2">Lunes 10:00 AM en Salón A y Salón B</p>
                            </div>
                        </div>
                        <div class="shrink-0">
                            <button class="flex min-w-[84px] max-w-[480px] cursor-pointer items-center justify-center overflow-hidden rounded-lg h-9 px-4 bg-gray-100 text-gray-800 hover:bg-gray-200 text-sm font-medium leading-normal w-fit">
            ...
            <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                <h2 class="text-gray-900 text-lg font-bold leading-tight tracking-[-0.015em] px-6 pt-5 pb-3">Próximas Clases</h2>
        ...
        <div class="flex flex-col gap-6">
            ...
            <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6 flex flex-col gap-4 h-fit">
                ...
                <div class="flex justify-center gap-6 text-sm">
                    <div class="flex items-center gap-2"><span class="size-3 rounded-full bg-primary"></span><span class="text-gray-600">Asignados</span></div>
                    <div class="flex items-center gap-2"><span class="size-3 rounded-full bg-gray-200"></span><span class="text-gray-600">Disponibles</span></div>
                </div>
            </div>

            <!-- Social Service Widget -->
            <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6 flex flex-col gap-4">
                <h2 class="text-gray-900 text-lg font-bold leading-tight tracking-[-0.015em]">Resumen de Servicio Social</h2>
